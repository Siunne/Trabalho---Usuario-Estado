...
public class UsuarioCadastrar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int id = Integer.parseInt(valorOuPadrao(request.getParameter("id"), "0"));
            String nome = valorOuPadrao(request.getParameter("nome"), "").trim();
            String cpf = valorOuPadrao(request.getParameter("cpf"), "").replaceAll("[^\\d]", "");
            String email = valorOuPadrao(request.getParameter("email"), "").trim();
            String senha = valorOuPadrao(request.getParameter("senha"), "").trim();
            String dataNascimentoParam = valorOuPadrao(request.getParameter("datanascimento"), "").trim();
            String salarioStr = valorOuPadrao(request.getParameter("salario"), "0");
            ...
            if (nome.isEmpty() || cpf.isEmpty() || dataNascimentoParam.isEmpty()
                    || email.isEmpty() || senha.isEmpty() || salario <= 0) {
                response.getWriter().write("5");
                return;
            }

            Date dataNascimento = java.sql.Date.valueOf(dataNascimentoParam);
